...
- **安装**
  ...
  【2】Windows
      方法1>  cmd命令行 -> python -m pip install requests(python -m 是获取管理员权限，ｍ是manage)
...
- **重大问题解决 - headers参数**
  ...
  养成好习惯，发送请求携带请求头，重构User-Agent（常见浏览器的user_agent百度）
  """
  import requests
...
- **示例代码**
  ...
  ```python
  re的compile方式
  pattern=re.compile('AB',re.S)
  pattern.findall('ABABA')
  优势：pattern可以复用
