...
​	API：Application Programming Interface  是一些预先定义的函数，或指软件系统不同组成部分衔接的约定。在web项目中为url。
...
​	1，base64 '防君子不防小人' （防止不可见字符（控制字符）在网络传输中产生错误（偏差）。**base64中所有的字符都是可见字符，是为了保证传输的正确性。**
