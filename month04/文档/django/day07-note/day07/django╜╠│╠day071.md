...
​	**Etag是服务器响应请求时，返回当前资源文件的一个唯一标识(由服务器生成)，只要资源有变化，Etag就会重新生成，HASH算法产生唯一标识，第一个特性：不可逆；第二个特性：输入敏感性，雪崩效应，用在对数据的完整性校验。数据的微小修改都会造成HASH值的完全不同。**
...
-  **为了防止别人拿到token，django会每次产生一个新token。**
