...
```shell
样例：
    博客发表文章时，提交XSS代码，服务器存储代码后，其他用户访问该文章时，被XSS攻击（避免方式：进行转义，html.escape('js脚本')
In [1]: import html
In [2]: script='<script>alert(111)</script>'
In [3]: safe_script=html.escape(script)
In [4]: safe_script
Out[4]: '&lt;script&gt;alert(111)&lt;/script&gt;'
In [5]: script
Out[5]: '<script>alert(111)</script>'
In [6]: html.unescape(safe_script)
...
3. 常用的过滤器
    ...
    | ... | **中文模式下，截取末尾不会加‘ .......’，英文下有8个字符，实际是7个，最后一个是‘....'** |
